...
export function ColorPicker({ onApply }: ColorPickerProps) {
  ...
  return (
    <div ref={ref} className="relative">
      <button type="button" onClick={() => { setOpen(p => !p); setInputVal(selected); }}
        title="Text Color"
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1.5 z-[100] bg-zinc-900 border border-zinc-700/70 rounded-xl p-3 shadow-2xl w-[220px]"
          ...
          <div className="flex items-center gap-1.5">
            ...
            <input type="text" value={inputVal} maxLength={7} spellCheck={false}
              onChange={e => { setInputVal(e.target.value); if (isValidHex(e.target.value)) setSelected(e.target.value); }}
              placeholder="#rrggbb"
